...
public class EntityProxyFactory implements ILoggingControl {
    ...
    /**
     * Creates a compound shape for a player entity.
     * Uses a capsule-like shape for better collision.
     */
    private CollisionShape createPlayerShape(PlayerEntity player) {
        ...
        // Create a narrower box for the body
        float bodyWidth = width * 0.7f;
        float bodyDepth = depth * 0.7f;
        BoxShape bodyShape = new BoxShape(new Vector3f(bodyWidth * 0.5f, height * 0.5f, bodyDepth * 0.5f));

        // Add body to compound shape
